...
function Paddle() {
  ...
  const [ref, api] = useBox(() => ({ type: "Kinematic", args: [0.17, 0.05, 0.175], onCollide: (e) => pong(e.contact.impactVelocity) }))
...
function Ball() {
  ...
  const [ref] = useSphere(() => ({
    mass: 0.1,
    args: 0.1,
    position: controller ? [controller.grip.position.x, controller.grip.position.y + 0.5, controller.grip.position.z - 0.1] : [0, 0, 0],
  }))
...
function ContactGround() {
  ...
  const [ref] = usePlane(() => ({ type: "Static", rotation: [-Math.PI / 2, 0, 0], position: [0, -2, 0], onCollide: () => reset(true) }))
  return <mesh ref={ref} />
}
